...
export function SiteTheme() {
    ...
    const { theme, colorBlindMode, colorGrayMode, themeRadius, changeSiteTheme, setPreferences } =
        usePreferencesStore();
                ...
                {themePresets.map((item) => (
                    <li key={item.type} onClick={() => handleClick(item.type)}>
                        <dl className="mb-0">
                            <dd
                                className={cn(
                                    'relative py-4 px-10 outline-solid outline-1 outline-gray-300 dark:outline-gray-700 rounded-md cursor-pointer',
